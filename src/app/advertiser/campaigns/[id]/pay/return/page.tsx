 ...
 * with "Try payment again" and "Check payment status".
 */
...
function ReturnInner({ id }: { id: string }) {
    ...
    return (
        <StepPage id={id} step={4} back={null} title={() => heading} subtitle={(ready) => `${accountNameOf(ready.advertiser)} · ${ready.campaign.name}`}>
            {(ready) => <Checking key={`${ready.campaign.id}:${paymentId ?? ""}`} ready={ready} paymentId={paymentId} onHeading={setHeading} />}
        </StepPage>
    );
}
...
function Checking({ ready, paymentId, onHeading }: { ready: ReadyCampaign; paymentId: string | null; onHeading: (heading: string) => void }) {
    ...
    React.useEffect(() => {
        ...
        return () => poll.stop();
    }, [wanted, round, campaign.id, router, onHeading]);

    const facts = [
        { label: "Payment reference", value: payment?.reference ?? campaign.reference },
        { label: "Campaign", value: campaign.name },
        { label: "Amount", value: rupees(payment?.amount ?? campaign.total ?? null) },
    ];
    const support = (
    ...
    return (
        <BookingCard className="py-8">
            <StatusPanel tone="amber" headline={phase.kind === "unsettled" ? "Still waiting for the bank." : "We're checking your payment."} lines={phase.kind === "unsettled" ? ["ADX has not heard from the gateway yet. If you completed the payment, it will settle on its own; check again in a minute.", "Please don't make another payment for this campaign yet."] : ["Keep this page open while we confirm the result.", "Please don't make another payment for this campaign yet."]} />
            <div className="mx-auto mt-6 max-w-[963px]">
                <FactRows rows={facts} />
                <div className="mt-6 flex flex-wrap justify-center gap-3">
                    {remembered?.url && (
